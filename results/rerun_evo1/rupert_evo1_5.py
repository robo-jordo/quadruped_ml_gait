...
def fitness(guess):
	...
	for i in range(len(guess)):
		...
		if(i%12==0):
			#print(i)
			hip1.publish((guess[i]/6.0)*limit)
			hip2.publish((guess[i+1]/6.0)*limit)
			hip3.publish((guess[i+2]/6.0)*limit)
			hip4.publish((guess[i+3]/6.0)*limit)
			knee1.publish((guess[i+4]/6.0)*limit)
			knee2.publish((guess[i+5]/6.0)*limit)
			knee3.publish((guess[i+6]/6.0)*limit)
			knee4.publish((guess[i+7]/6.0)*limit)
			ankle1.publish(((guess[i+8])/6.0)*limit)
			ankle2.publish(((guess[i+9])/6.0)*limit)
			ankle3.publish(((guess[i+10])/6.0)*limit)
			ankle4.publish(((guess[i+11])/6.0)*limit)
			rospy.sleep(1.2)
	performance = final_position - initial_position + height_average/height_count - pitch_average/height_count - roll_average/height_count
	print("check: " + str(pitch_average/height_count))
	rec_count = 0
	#unload_controllers()
	delete_model("rupert")
