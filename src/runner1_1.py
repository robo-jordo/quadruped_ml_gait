...
best = [1, -2, -2, 6, 6, -2, 1, 3, -5, -2, 3, 2, 6, -6, 1, 4, 4, 2, -5, 5, 3, -4, 2, -2, -4, 3, 3, -5, -3, -3, 1, 4, 4, 6, -6, -3, 4, -5, 2, -6, -4, 1, -5, -5, -4, 2, -4, 0, 3, 0, 5, 6, 5, -6, 0, 0, -3, -4, -4, -6, -6, 5, 1, -5, 4, 6, 6, 2, -5, -6, 5, -2, -1, 6, -4, -2, 2, -5, 1, 2, 4, 0, 5, 4, 0, 0, -2, -1, 2, -2, -6, -1, -5, -2, -3, 0, 3, -4, 4, -4, 3, 6, -4, 0, -2, -1, -2, -3, -6, 4, -3, -6, 3, -1, -5, -6, -6, -2, 0, -6, -6, -6, 6, 6, -1, 3, 2, 5, -5, 2, 6, 0, 1, -1, -2, 2, -2, 4, -6, -5, 6, -2, -1, 4, -3, -4, -3, -3, -3, -2, 1, 1, 2, 2, 6, 6, -6, -6, 2, 4, -6, -2, -2, 1, 5, 4, 5, -3, -4, 3, -1, 6, -1, -3, -6, -2, 1, 3, -2, 2, 1, 3, 0, -5, -1, 5, 6, 5, 2, 6, -2, -3]
limit = 1.6
...
def main():
	...
	for j in range(5):
		for i in range(len(best)/4):
			if(i%8==0):
				#print(i)
				hip1.publish((best[i]/6.0)*limit)
				hip2.publish((best[i+1]/6.0)*limit)
				hip3.publish((best[i+2]/6.0)*limit)
				hip4.publish((best[i+3]/6.0)*limit)
				knee1.publish((best[i+4]/6.0)*limit)
				knee2.publish((best[i+5]/6.0)*limit)
				knee3.publish((best[i+6]/6.0)*limit)
				knee4.publish((best[i+7]/6.0)*limit)
				ankle1.publish(((best[i+8])/6.0)*limit)
				ankle2.publish(((best[i+9])/6.0)*limit)
				ankle3.publish(((best[i+10])/6.0)*limit)
				ankle4.publish(((best[i+11])/6.0)*limit)
				rospy.sleep(1.2)
